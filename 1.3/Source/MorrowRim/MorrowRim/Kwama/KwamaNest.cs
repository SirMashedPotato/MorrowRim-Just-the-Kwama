...
namespace MorrowRim.Kwama
{
    public class KwamaNest : ThingWithComps
    {
		...
		/*
		int ticks = 0;
		public void CheckInsectoids()
        {
            if (ticks >= 100)
            {
				if (GenRadial.RadialDistinctThingsAround(this.Position, this.Map, 4f, true).Where(x => x is Pawn && x.Faction != null && x.Faction == RimWorld.Faction.OfInsects).Any())
                {
					Lord lord = base.GetComp<CompSpawnerPawn>().Lord;
					if (lord != null)
					{
						lord.ReceiveMemo(KwamaNest.MemoInsectoids);
					}
				}
				ticks = 0;
            }
		...
		*/
		...
		public static readonly string MemoInsectoids = "KwamaNestTrojan";
	}
}
