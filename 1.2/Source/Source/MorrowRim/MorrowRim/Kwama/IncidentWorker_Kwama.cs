...
namespace MorrowRim_Kwama.Kwama
{
    class IncidentWorker_Kwama : IncidentWorker
    {
		...
		protected override bool CanFireNowSub(IncidentParms parms)
		{
			...
			IntVec3 intVec;
			return base.CanFireNowSub(parms) && KwamaNestUtility.TotalSpawnedHivesCount(map) < 30 && InfestationCellFinder.TryFindCell(out intVec, map);
		}
		...
		{
			Map map = (Map)parms.target;
			Thing t = KwamaNestUtility.SpawnTunnels(Mathf.Max(GenMath.RoundRandom(parms.points / 220f), 1), map, false, false, null);
